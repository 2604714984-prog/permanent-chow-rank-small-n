...
\author{Rongyu Xu}
\hypersetup{
  pdftitle={The Chow Rank of the Six-by-Six Permanent},
  pdfauthor={Rongyu Xu}}
...
ordinary Chow rank is \(32\). The finite computations are stated as explicit
lemmas with exhaustive domains and exact
outputs. The lower bound combines a
symmetric image-span inequality with a half-defect estimate for arbitrary
quotient derivative symbols of a sextic Chow term. An exact verification
...
coordinate verification of the small permanent-quadratic intersections. The
verification reconstructs these objects from their definitions and compares
them with the exact payload. All geometric reductions and global inequalities
are written proofs.
...
The independent program \path{scripts/n6_independent_finite_core_verification.py}
...
the independent computation uses only this direct survival test and does not import
...
python scripts/n6_independent_finite_core_verification.py
...
third command is independent of that implementation and of the reference JSON;
...
written proofs rather than numerical inferences.
