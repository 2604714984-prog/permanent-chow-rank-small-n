...
\author{Rongyu Xu}
\hypersetup{
  pdftitle={The Chow Rank of the Five-by-Five Permanent},
  pdfauthor={Rongyu Xu}}
...
\(\Q\); independent programs also verify Glynn's identity and all
\(\binom{25}{5}\) coordinate five-planes. A deterministic fail-closed replay
links these certificates to the written proof.
\end{abstract}
...
For direct verification, the complete state list and its unique route assignment are
...
visible-shadow, and inverse-compression arguments in
Proposition~\ref{prop:terminalrank} reduce the terminal calculation to orbit
zero, where \(\rank K(R)\ge2215\).
\end{lemma}

\begin{proof}[Certificate proof]
The independent parent-table verification streams, rather than stores, all
\(\binom{25}{5}=53{,}130\) coordinate five-planes and recomputes the four
maxima. The entry point
\path{scripts/perm5_terminal_independent_verification.py} reconstructs the tangent
graph and rational Fourier matrices directly from their definitions and
checks the terminal rank gap. The exact finite inputs and outputs are listed in
\path{docs/computational_lemmas.md}.
\end{proof}
...
map with the permanent support. This is a direct verification of the displayed
...
\section{Exact verification}\label{sec:replay}

From the repository root, execute
\begin{verbatim}
python -B perm5/verify_all.py
\end{verbatim}
The command requires Python 3 and only its standard library. It performs five
...
\item the reconstruction and unique route assignment of all fifty-eight
fixed-six states;
\item the streamed scan of all \(\binom{25}{5}=53{,}130\) coordinate
five-planes, reproducing the parent-table maxima \(4,4,5,7\);
\item the \(4{,}100\)-vertex tangent graph and rational Fourier computation,
which give \(2215>2205\); and
\item the \(50{,}000\)-choice integer expansion of Glynn's identity.
\end{enumerate}
The map from finite lemmas to programs and exact outputs is given in
\path{docs/computational_lemmas.md}. The verifier fails on a missing file, a
changed payload, or a nonzero computation exit. The theorem concerns ordinary
Chow rank; it makes no border Chow-rank statement and no equality claim for
general \(n\).
